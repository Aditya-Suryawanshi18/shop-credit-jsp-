...
<head>
    ...
    <style>
        .product-tag { background:#e8f0fe; color:#1a56db; padding:3px 10px; border-radius:20px; font-size:12px; font-weight:600; white-space:nowrap; }
        .product-tag.settle { background:#fef3e2; color:#b45309; }
        .qty-pill { display:inline-block; background:#e8f5e9; color:#1b5e20; border:1px solid #a5d6a7; border-radius:20px; padding:2px 12px; font-size:12px; font-weight:700; }
        ...
        .print-controls input[type="date"]:focus { border-color: #7a3800; }
        .btn-print {
            padding: 8px 18px;
        ...
        .btn-print-all {
            ...
            color: #7a3800; border: 1.5px solid #7a3800; border-radius: 8px;
...
<div class="content-wrapper">
    ...
    <!-- Dealer Info Card -->
    <div class="detail-info-card no-print" style="border-left-color: #f5a623;">
        <div class="info-item"><span class="info-label">Dealer ID</span><span class="info-value">#<%= dealerId %></span></div>
        <div class="info-item"><span class="info-label">Dealer Name</span><span class="info-value">🏬 <%= dealerName %></span></div>
        <div class="info-item"><span class="info-label">Phone</span><span class="info-value">📞 <%= dealerPhone %></span></div>
        <div class="info-item address-item"><span class="info-label">Address</span><span class="info-value">📍 <%= dealerAddress %></span></div>
        <div class="info-item"><span class="info-label">Current Credit</span><span class="info-value credit-amount">₹ <%= String.format("%.2f", dealerCredit) %></span></div>
    </div>
    ...
    <div class="stats-row no-print">
        ...
        <div class="stat-chip"><div class="s-label">Net Credit</div><div class="s-value" style="color:#2b0d73;">₹ <%= String.format("%.2f", dealerCredit) %></div></div>
    </div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Product / Item</th><th>Stock Added</th><th>Payment Mode</th><th>Amount (₹)</th>
                </tr>
            </thead>
            <tbody>
            <%
            ...
            <%
                    ...
                    if (!hasTxn) {
            %>
                <tr><td colspan="8" class="no-data">No transactions found for this dealer.</td></tr>
            <%  }
                } catch (Exception e) { %>
...
<%
    try (Connection conn = DBConnection.getConnection()) {
        PreparedStatement ps = conn.prepareStatement(
            ...
            "FROM dealer_transactions WHERE dealer_id = ? ORDER BY transaction_date DESC");
        ps.setInt(1, dealerId);
        ResultSet rs = ps.executeQuery();
        boolean first = true;
        while (rs.next()) {
...
var DEALER_NAME   = "<%= jsEscape(dealerName) %>";
var DEALER_ID     = "<%= dealerId %>";
var DEALER_PHONE  = "<%= jsEscape(dealerPhone) %>";
var DEALER_ADDR   = "<%= jsEscape(dealerAddress) %>";
var DEALER_CREDIT = "<%= String.format("%.2f", dealerCredit) %>";
...
function buildPopupHtml(txns, periodLabel, dateStr) {
    ...
    txns.forEach(function(t) {
        ...
        rows += '<tr style="' + bg + '">' +
            ...
            '<td style="padding:8px 10px;border-bottom:1px solid #e2e8f0;">' + (t.displayDate || t.date) + '</td>' +
    ...
    return '<!DOCTYPE html><html><head><meta charset="UTF-8">' +
        '<title>Dealer Statement - ' + DEALER_NAME + '</title>' +
        '<style>' +
        'body{font-family:Arial,sans-serif;margin:30px;color:#0d1b2a;font-size:13px;}' +
        '.header{text-align:center;border-bottom:3px double #7a3800;padding-bottom:14px;margin-bottom:16px;}' +
        ...
        '.info-box{display:grid;grid-template-columns:1fr 1fr;gap:6px 24px;background:#fff8f0;border:1px solid #f5dab0;padding:12px 16px;margin-bottom:14px;border-radius:4px;}' +
        ...
        '.info-value.credit{color:#c2730a;font-size:15px;}' +
        ...
        'th{background:#7a3800;color:#fff;padding:8px 10px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:0.5px;}' +
        'tfoot td{background:#fff3e0;font-weight:700;border-top:2px solid #7a3800;padding:9px 10px;}' +
        ...
        '</div>' +
        '<div class="info-box">' +
        '<div class="info-item"><span class="info-label">Dealer Name</span><span class="info-value">' + DEALER_NAME + '</span></div>' +
        '<div class="info-item"><span class="info-label">Dealer ID</span><span class="info-value">#' + DEALER_ID + '</span></div>' +
        '<div class="info-item"><span class="info-label">Phone No.</span><span class="info-value">' + DEALER_PHONE + '</span></div>' +
        '<div class="info-item"><span class="info-label">Address</span><span class="info-value">' + DEALER_ADDR + '</span></div>' +
        '<div class="info-item"><span class="info-label">Current Credit Balance</span><span class="info-value credit">&#8377; ' + DEALER_CREDIT + '</span></div>' +
        '</div>' +
        '<div class="summary">' +
